/* PYF writes all */
/* finally finished on May 20th 17:15 */
...
/* -------------------------------------------------------- */
/* also a change in line 629 to run pass optimization */
/* optimization part, optional choices for optimization,
   in llvm each pass is independent from others, you could add 
   any of them but it may also cause bugs, so be careful! */
void InitializeModuleAndPassManager() {
  ...
  /* 1 Promote allocas of memory to registers. */
  //TheFPM->add(createPromoteMemoryToRegisterPass());

  /* 2 Do simple "peephole" optimizations and bit-twiddling optzns. */
  /* this is kind of like making x=x+1; x=x+1 to x=x+2; */
  /* to learn more, you could see the official comments or search in the internet */
  TheFPM->add(createInstructionCombiningPass());
  
  /* 3 Reassociate expressions. */
  /* for example, know (x+3) and (3+x) are the same to avoid repeating */
  TheFPM->add(createReassociatePass());
  
  /* 4 Eliminate Common SubExpressions. */
  /* eliminate some expressions not used, always coordinating with the one above */
  //TheFPM->add(createGVNPass());

  /* 5 Simplify the control flow graph (deleting unreachable blocks, etc). */
  /* may cause errors */
...
/* -------------------------------------------------------- */
/* initialize an output declaration for i16 to output */
/* this function IR will be linked in a .ll file compiled by clang */
void InitializeCFunctions(){
    std::vector<std::string> argus;
    argus.push_back("X");
    std::vector<std::string> types;
    types.push_back("int");
    auto Proto = std::make_unique<PrototypeAST>("INT",  "printi",
                                 std::move(argus),std::move(types));
